...
mRNA <- subset(gff, gff$feature == 'mRNA') %>%
  ...
  mutate(bio=getAttributeField(attributes, 'biotype')) %>%
  select(c('seqname','start','end','bio','Name','score','strand'))
mRNA <- mRNA[!duplicated(mRNA$Name),] # this keeps primary
...
exon <- subset(gff, gff$feature == 'exon') %>%
  ...
  select(c('seqname','start','end','bio','Name','score','strand'))%>%
  na.omit()
exon <- exon[!(duplicated(exon$start)&duplicated(exon$end)),]
...
cds <- cds[!(duplicated(cds$start)&duplicated(cds$end)),]
...
utr_5 <- subset(gff, feature == "five_prime_UTR") %>%
  ...
  mutate(bio=feature) %>%
  select('seqname','start','end','bio','Name','score','strand')
utr_5 <- utr_5[!(duplicated(utr_5$start)&duplicated(utr_5$end)),]
...
utr_3 <- subset(gff, feature == "three_prime_UTR") %>%
  ...
  mutate(bio=feature) %>%
  select('seqname','start','end','bio','Name','score','strand')
utr_3 <- utr_3[!(duplicated(utr_3$start)&duplicated(utr_3$end)),]
